...
option = {
	...
	geo : {
		...
		left : '2%',
		right : '35%',
		center : [107.98561551896913, 36.205000490896193],
		zoom : 0.9,
		label : {
			emphasis : {
				show : true,
...
function initTop10() {
	...
	$.getJSON('http://127.0.0.1:8000/api/' + datenum, function (data) {
		...
		var convertedData = [convertData(data.sort(function (a, b) {
					return b.value - a.value;
				}).slice(0, maxBar + 2)), convertData(data.sort(function (a, b) {
					return b.value - a.value;
				}).slice(maxBar + 2))];
		for (var i = 0; i < Math.min(convertedData[0].length, maxBar); i++) {
			categoryData.push(convertedData[0][i].name);
			barData.push(convertedData[0][i].value[2]);
		}
		myChart.setOption({
			title : [{
					subtext : dateshownum,
